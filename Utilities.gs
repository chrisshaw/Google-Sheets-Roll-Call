...
function findRowBy2(data, timeField, timeMatch, field2, match2, headerRow){
/*The first field is always time*/
  var startRow = headerRow + 1;
  var foundRow = -1
  for(var i = startRow; i < data.length; i++){
    var dataStamp = data[i][timeField];
    var dataTimeCode = dataStamp.getDate() + dataStamp.getMonth() + dataStamp.getYear();
    var matchTimeCode = timeMatch.getDate() + timeMatch.getMonth() + timeMatch.getYear();
    Logger.log(dataTimeCode + '---' + matchTimeCode);
    if(dataTimeCode === matchTimeCode){
      if(data[i][field2] === match2){      
        foundRow = i+1;
        return foundRow;
      }
    }
